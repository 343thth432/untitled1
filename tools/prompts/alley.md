...
Двенадцать кадров, тварь закрыта полностью:

- ракурсы: анфас, три четверти, профиль, три четверти со спины, спина;
- шаг: четыре фазы;
- бой: замах и удар;
- боль;
- смерть: оседает, на коленях, лежит.

Что выяснилось по дороге и пригодится для следующих тварей:

- **лист удобнее отдельных картинок.** Персонаж выходит одинаковым во
  всех кадрах, и спрайт с него даже резче: при уменьшении в восемь раз
  линии замыливаются, а в четыре держатся.
- **отдельный кадр приходит в своём масштабе.** Генератор вписывает
  фигуру в кадр, а не держит расстояние до камеры. Поправка кладётся в
  `scale.json`, множитель снимается сравнением голов при равной высоте
  рамки — голова от позы не зависит. Сверять надо с кадром той же осанки:
  стойка с широко расставленными ногами ниже обычной на восемь процентов.
- **фон может прийти шахматкой** вместо прозрачности. Это не беда:
  конвейер срезает фон заливкой от углов, а не по цвету, поэтому светлая
  блузка не страдает.
- **фигуры на листе могут соприкоснуться** носком ботинка или хвостом.
  Резчик их разлепляет сам.

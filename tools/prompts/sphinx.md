...
> **Отработано, босс в игре.** Лист пришёл на восемнадцать фигур: пять
> ракурсов, четыре шага, четыре кадра боя и четыре падения. Из них взяты
> семнадцать, один лежачий — почти копия соседнего. Бросок расписан
> двумя кадрами: набирает (хвосты веером, голова опущена) и выбрасывает
> (встаёт на дыбы, лапы вверх, рёв). Под это в раскадровку добавлена
> поза `cast1`. Что стоит в игре — смотри `public/art/foes/sphinx/` и
> запись `sphinx` в `src/dungeon/foes.ts`.
